// class for storing all data relating to a single flight, written by Shane Byrd, edited by Tommy (Zhihan)
//
...
class Flight {
  ...
  Flight(String line) {
    ...
    if (line.charAt(line.length()-3) == '.') {
      distance = parseIntRange(line, c18 + 1, line.length()-3);
    }
    else {
  ...
  void parseDate(String s, int start) {
    ...
    if (s.charAt(start+1) == '/') {
      // first month is 1/ and not 01/
      flightMonth = s.charAt(start) - '0';
      if (s.charAt(start + 4) == '/') {
        // if in format 1/01/2022
        flightDay = (s.charAt(start + 2) - '0') * 10 + (s.charAt(start + 3) - '0');
        flightYear = (s.charAt(start + 5) - '0') * 1000
          + (s.charAt(start + 6) - '0') * 100
          + (s.charAt(start + 7) - '0') * 10
          + (s.charAt(start + 8) - '0');
      }
      else {
        // if in format 1/1/2022
        flightDay = s.charAt(start + 2) - '0';
        flightYear = (s.charAt(start + 4) - '0') * 1000
          + (s.charAt(start + 5) - '0') * 100
          + (s.charAt(start + 6) - '0') * 10
          + (s.charAt(start + 7) - '0');
      }
      ...
      if (s.charAt(start + 5) == '/') {
        ...
        //  if in format 01/1/2022
        flightDay = (s.charAt(start + 3) - '0') ;
        flightYear = (s.charAt(start + 5) - '0') * 1000
          + (s.charAt(start + 6) - '0') * 100
          + (s.charAt(start + 7) - '0') * 10
          + (s.charAt(start + 8) - '0');
