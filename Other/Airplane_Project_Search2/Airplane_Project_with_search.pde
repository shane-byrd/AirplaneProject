...
void setup() 
{
  ...
  graphTypeMenu = new DropDown(
    1015, 40, 170, 30, "graphType", "Graph: Bar",
    color(#5a90d6),
    color(#000000),
    smallFont,
    3, 6,
  ...
  graphTypeMenu.addButton("bar", "Bar");
  graphTypeMenu.addButton("histogram", "Histogram");
  graphTypeMenu.addButton("scatter", "Scatter");
...
void draw() 
{
  ...
  graphTypeMenu.textLabel = "Graph: " + selectedGraphType;
  ...
  text("Results: " + filteredFlights.size(), 1015, 75);
}
...
void mousePressed() {
  ...
  if (graphTypeMenu.openW) {
    String overButton = graphTypeMenu.whichButtonOver();
    if (overButton.equals("None")) {
      graphTypeMenu.openW = false;
    } else {
      if (overButton.equals("bar")) {
        selectedGraphType = "Bar";
        graph.setGraphType("bar");
      }
      if (overButton.equals("histogram")) {
        selectedGraphType = "Histogram";
        graph.setGraphType("histogram");
      }
      if (overButton.equals("scatter")) {
  ...
  } else if (graphTypeMenu.cursorOverWidget()) {
    graphTypeMenu.openW = true;
    return;
  }
...
class Graph {
  ...
  ArrayList<Float> scatterX;
  ArrayList<Float> scatterY;
  int scatterCount;
  ...
  void buildHistogramData() {
    ...
    int minDist = flights.get(0).distance;
    int maxDist = flights.get(0).distance;

    for (Flight f : flights) {
      if (f.distance < minDist) minDist = f.distance;
      if (f.distance > maxDist) maxDist = f.distance;
    }

    int startBin = (minDist / histBinSize) * histBinSize;
    int endBin = ((maxDist / histBinSize) + 1) * histBinSize - 1;
    histCount = ((endBin - startBin + 1) + histBinSize - 1) / histBinSize;

    histBinStarts = new int[histCount];
    histBinEnds = new int[histCount];
    histBinCounts = new int[histCount];

    for (int i = 0; i < histCount; i++) {
      histBinStarts[i] = startBin + i * histBinSize;
      histBinEnds[i] = histBinStarts[i] + histBinSize - 1;
      histBinCounts[i] = 0;
    }

    for (Flight f : flights) {
      int index = (f.distance - startBin) / histBinSize;
  ...
  void buildScatterData() {
    scatterX = new ArrayList<Float>();
    scatterY = new ArrayList<Float>();

    for (Flight f : flights) {
      if (f.depTime > 0 && f.distance > 0) {
        scatterX.add(float(f.depTime));
        scatterY.add(float(f.distance));
      }
    }

    scatterCount = scatterX.size();
  }
  ...
  void draw() {
    ...
    if (graphType.equals("bar")) {
      ...
      text("Histogram of Flight Distance", x + w/2, y + 25);
      drawHistogram();
    } else if (graphType.equals("scatter")) {
      text("Scatter Plot: Departure Time vs Distance", x + w/2, y + 25);
      drawScatterPlot();
    }
  ...
  void drawBarChart() {
    ...
    for (int i = 0; i < barCount; i++) {
      if (airportCounts[i] > maxCount) maxCount = airportCounts[i];
    }
  ...
  void drawHistogram() {
    if (histCount == 0) {
      fill(0);
      textFont(labelFont);
      text("No histogram data available", x + w/2, y + h/2);
      return;
    }

    int maxCount = 0;
    for (int i = 0; i < histCount; i++) {
      if (histBinCounts[i] > maxCount) maxCount = histBinCounts[i];
    }
    ...
    float barW = chartW / histCount;

    for (int i = 0; i < histCount; i++) {
      float barX = chartX + i * barW;
      float barH = map(histBinCounts[i], 0, maxCount, 0, chartH - 20);
      float barY = chartY + chartH - barH;
      ...
      textFont(labelFont);
      textAlign(CENTER, TOP);
      text(histBinStarts[i], barX + barW/2, chartY + chartH + 8);
    }
  }
  ...
  void drawScatterPlot() {
    if (scatterCount == 0) {
      fill(0);
      textFont(labelFont);
      text("No scatter data available", x + w/2, y + h/2);
      return;
    }

    float minX = scatterX.get(0);
    float maxX = scatterX.get(0);
    float minY = scatterY.get(0);
    float maxY = scatterY.get(0);

    for (int i = 1; i < scatterCount; i++) {
      if (scatterX.get(i) < minX) minX = scatterX.get(i);
      if (scatterX.get(i) > maxX) maxX = scatterX.get(i);
      if (scatterY.get(i) < minY) minY = scatterY.get(i);
      if (scatterY.get(i) > maxY) maxY = scatterY.get(i);
    }
    ...
    textFont(labelFont);
    textAlign(CENTER, TOP);
    text("Departure Time", chartX + chartW/2, chartY + chartH + 35);

    pushMatrix();
    translate(chartX - 45, chartY + chartH/2);
    rotate(-HALF_PI);
    textAlign(CENTER, CENTER);
    ...
    for (int i = 0; i < scatterCount; i++) {
      float px = map(scatterX.get(i), minX, maxX, chartX, chartX + chartW);
      float py = map(scatterY.get(i), minY, maxY, chartY + chartH, chartY);
  ...
  void drawHoverInfo() {
    ...
    if (graphType.equals("bar")) {
      info = getBarHoverText();
    } else if (graphType.equals("histogram")) {
      info = getHistogramHoverText();
    } else if (graphType.equals("scatter")) {
      info = getScatterHoverText();
    }

    if (!info.equals("")) {
      fill(255, 245, 220);
      stroke(120);
      rect(10, SCREENY - 35, 330, 22);

      fill(0);
      textFont(labelFont);
      textAlign(LEFT, CENTER);
      text(info, 16, SCREENY - 24);
    }
  }

  boolean mouseInsideGraphBox() {
    return mouseX >= x && mouseX <= x + w && mouseY >= y && mouseY <= y + h;
  }

  String getBarHoverText() {
    ...
    for (int i = 0; i < barCount; i++) {
      ...
      if (mouseX >= barX && mouseX <= barX + currentBarW &&
        mouseY >= barY && mouseY <= chartY + chartH) {
        return "Y: " + round(airportCounts[i]) + " flights";
      }
    }
  ...
  String getHistogramHoverText() {
    ...
    for (int i = 0; i < histCount; i++) {
      ...
      if (mouseX >= barX && mouseX <= barX + barW &&
        mouseY >= barY && mouseY <= chartY + chartH) {
        return "X: " + round(histBinStarts[i]) + "-" + round(histBinEnds[i]) + " miles, Y: " + round(histBinCounts[i]) + " flights";
      }
    }
  ...
  String getScatterHoverText() {
    ...
    for (int i = 0; i < scatterCount; i++) {
      ...
      if (dist(mouseX, mouseY, px, py) <= 6) {
        return "X: " + convertTime24(round(scatterX.get(i))) + ", Y: " + round(scatterY.get(i)) + " miles";
      }
    }
